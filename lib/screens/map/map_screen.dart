...
class _FullMapScreenState extends State<FullMapScreen> {
  ...
  Future<void> _preCacheMarkerIcons() async {
    ...
    _defaultSafeIconCache =
        await _getBitmapFromIcon(Icons.shield_rounded, Colors.blue);
  }
  ...
  Future<void> _shareLiveLocationSOS() async {
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        int activeSosId =
            resData['data'] != null ? resData['data']['id'] : resData['id'];
    ...
    await _launchSosGuard(
        sosId: mockId, token: token ?? 'mock-token', isMock: true);
  }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        for (var z in zonesData) {
          ...
          bool isAutomatic =
              z['is_automatic'] == true || z['is_automatic'] == 1;
          bool notifyAndTriggerSos =
              z['notify_contacts'] == true || z['notify_contacts'] == 1;
          String type = z['type'].toString().toLowerCase();
          Color zoneColor =
              (type == 'high_alert' || type == 'danger' || type == 'red')
                  ? Colors.red
                  : (type == 'safe' || type == 'blue')
                      ? Colors.blue
                      : Colors.orange;
          ...
          Circle circle = Circle(
            circleId: CircleId(
                "server_${type}_${zoneId}_triggerSos_$notifyAndTriggerSos"),
          ...
          if (!isAutomatic) {
            _userCustomZones
                .removeWhere((element) => element['name'] == zoneName);
  ...
  Future<void> _saveZoneToServer(LatLng pos, double radius, String type,
      String name, bool notifyAndTriggerSos) async {
    String backendType;
    if (type.toLowerCase() == 'danger' ||
        type.toLowerCase() == 'red' ||
        type.toLowerCase() == 'high_alert') {
  ...
  Future<void> _loadDataFromLocal() async {
    ...
    if (recentsJson != null) {
      setState(() => _recentPlaces =
          List<Map<String, dynamic>>.from(json.decode(recentsJson)));
    }
    await _loadCustomZonesFromCache();
    _refreshMarkers();
  }
  ...
  Future<void> _saveCustomZonesToCache() async {
    final prefs = await SharedPreferences.getInstance();
    List<String> rawList = _userCustomZones
        .map((z) => json.encode({
              'id': z['id'].toString(),
              'name': z['name'],
              'isZone': true,
  ...
  void _refreshMarkers() {
    ...
    for (var entry in _safePlaces.entries) {
      BitmapDescriptor iconImage =
          _defaultSafeIconCache ?? BitmapDescriptor.defaultMarker;
      if (entry.key == "Home" && _homeIconCache != null) {
        iconImage = _homeIconCache!;
      }
      if (entry.key == "Work" && _workIconCache != null) {
        iconImage = _workIconCache!;
      }
  ...
  Future<BitmapDescriptor> _getBitmapFromIcon(
    ...
    textPainter.text = TextSpan(
      ...
      style: TextStyle(
          fontSize: 90.0, fontFamily: iconData.fontFamily, color: color),
    );
    textPainter.layout();
  ...
  void _onMapTap(LatLng position) async {
    if (_isSelectingZoneLocation) {
      ...
      if (!mounted) return;
      LocationSheets.showMarkLocationOptions(context, position, placeName,
          (radius, type, notifyAndTriggerSos) {
        _saveZoneToServer(
            position, radius, type, placeName, notifyAndTriggerSos);
        _addNewCircleLocally(
            position, radius, type, placeName, notifyAndTriggerSos);
      });
      _animateSheet(0.15);
  ...
  void _addNewCircleLocally(LatLng pos, double radius, String type, String name,
      bool notifyAndTriggerSos) {
    final CircleId tempId = CircleId(
        "temp_${type}_${DateTime.now().millisecondsSinceEpoch}_triggerSos_$notifyAndTriggerSos");
    bool isDangerZone =
        (type == 'danger' || type == 'high_alert' || type == 'red');
    final Circle newCircle = Circle(
      circleId: tempId,
      center: pos,
      radius: radius,
      fillColor: isDangerZone
              ...
              return Container(
                ...
                child: ListView(
                  ...
                  children: [
                    ...
                    _buildSectionHeader("Manage Places",
                        showArrow: true, onTap: () => _navToManagePlaces()),
                    _buildSafePlacesRow(),
                    const SizedBox(height: 25),
                    _buildSectionHeader("Recent Searches"),
                    ..._recentPlaces.asMap().entries.map(
                        (entry) => _buildRecentTile(entry.key, entry.value)),
                    ...
                    CustomButton(
                        text: _isSelectingZoneLocation
                            ? "Cancel Selection"
                            : "Mark New Zone",
                        onPressed: () {
                          setState(() {
                            _isSelectingZoneLocation =
                                !_isSelectingZoneLocation;
                            if (_isSelectingZoneLocation) {
    ...
    return GestureDetector(
      ...
      child: Column(children: [
        Container(
          ...
          child:
              Icon(icon, color: exists ? Colors.blue : Colors.grey, size: 30),
        ),
        const SizedBox(height: 8),
        Text(label,
            style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w500)),
        Text(exists ? "Saved" : "Add",
            style: TextStyle(
                fontSize: 12, color: exists ? Colors.green : Colors.blue)),
      ]),
    );
  }
  ...
  Future<String> _getAddressFromLatLng(LatLng position) async {
    ...
    try {
      final response = await http
          .get(Uri.parse(url), headers: {'User-Agent': 'VoxGuard_App_v1'});
  ...
  Future<void> _handleSearch(String input) async {
    ...
    try {
      ...
      if (data.isNotEmpty) {
        ...
        _moveTo(pos);
        _addToRecents(
            input, data[0]['display_name'], pos.latitude, pos.longitude);
        _refreshMarkers();
  ...
  void _addToRecents(String name, String addr, double lat, double lng) async {
    setState(() {
      ...
      _recentPlaces
          .insert(0, {'name': name, 'address': addr, 'lat': lat, 'lng': lng});
    });
    final prefs = await SharedPreferences.getInstance();
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: TextField(
            ...
            decoration:
                const InputDecoration(hintText: "Place name (e.g., Gym)")),
        actions: [
          TextButton(
              onPressed: () {
                setState(() {
...
Widget _buildSuggestionCard() {
  return Container(
    ...
    child: ListTile(
      leading: CircleAvatar(
          backgroundColor: _suggestedName == "Searching nearby..." ? Colors.grey : Colors.blue,
          child: _suggestedName == "Searching nearby..." 
              ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white))
              : Icon(_suggestionIcon, color: Colors.white)),
  ...
  void _navToManagePlaces() async {
    final bool? shouldStartPicking = await Navigator.push(
        context,
        MaterialPageRoute(
            builder: (context) => ManagePlacesScreen(
                  places: _safePlaces,
                  ...
                  onDelete: (key) {
                    ...
                    SharedPreferences.getInstance().then((prefs) {
                      prefs.setString(
                          'safePlaces',
                          json.encode(_safePlaces.map((k, v) => MapEntry(
                              k, {'lat': v.latitude, 'lng': v.longitude}))));
                    });
                  },
                  onDeleteZone: (circleIdStr) {
                    setState(() {
                      _circles
                          .removeWhere((c) => c.circleId.value == circleIdStr);
                      _userCustomZones
                          .removeWhere((z) => z['id'] == circleIdStr);
                    });
                    _saveCustomZonesToCache();
                  },
...
if (_userCustomZones.isNotEmpty) {
  var safeZones = _userCustomZones.where((zone) => zone['isDanger'] == false).toList();
  
  for (var zone in safeZones) {
    double zLat = zone['lat'];
    double zLon = zone['lng'];
    double dist = Geolocator.distanceBetween(lat, lon, zLat, zLon);
    
    if (dist <= 500) { 
      if (mounted) {
        setState(() {
        _suggestedName = zone['name'] ?? "My Safe Zone";
        _suggestedLocation = LatLng(zLat, zLon);
        _suggestedDist = "Your safe zone: ${dist.toInt()}m away";
        _suggestionIcon = Icons.shield_rounded; 
      });
      }
      return;
    }
  ...
  // 3. الأولوية الثالثة: البحث في خرائط العالم (OpenStreetMap)
  ...
  try {
    ...
    if (data['elements'] != null && (data['elements'] as List).isNotEmpty) {
      ...
      var nearest = elements.reduce((curr, next) {
        double distCurr = Geolocator.distanceBetween(lat, lon, (curr['lat'] ?? curr['center']['lat']).toDouble(), (curr['lon'] ?? curr['center']['lon']).toDouble());
        double distNext = Geolocator.distanceBetween(lat, lon, (next['lat'] ?? next['center']['lat']).toDouble(), (next['lon'] ?? next['center']['lon']).toDouble());
        return distCurr < distNext ? curr : next;
      });

      LatLng targetLoc = LatLng((nearest['lat'] ?? nearest['center']['lat']).toDouble(), (nearest['lon'] ?? nearest['center']['lon']).toDouble());
