...
class _IncomingFakeCallPoliceState extends State<IncomingFakeCallPolice> {
  ...
  Future<void> _answerCall() async {
    ...
    try {
      ...
      final String tempPath = "${tempDir.path}/${ApiConfig.policeVoice.split('/').last}";
      final File file = File(tempPath);
      if (!await file.exists()) {
        await dio.download(ApiConfig.policeVoice, tempPath);
      }
      await _audioPlayer.setSource(DeviceFileSource(tempPath));
      await _audioPlayer.resume();
    } catch (e) {
      debugPrint('Error playing police voice from API, trying fallback: $e');
      try {
        await _audioPlayer.setSource(UrlSource(ApiConfig.policeVoice));
        await _audioPlayer.resume();
      } catch (e2) {
        debugPrint('Fallback Police play failed: $e2');
      }
    }
  }
